...
class file
{
    ...
    private function get_filename($key)
    {
        ...
        //得到文件缓存路径
        $file_cache_path = ROOT_PATH . \core\plugin\Config::get('cache', 'FILE_CACHE')['PATH'];
        if (!file_exists($file_cache_path)) {
            $create_file_cache_path = mkdir($file_cache_path, 0777, true);
            if (!$create_file_cache_path) {
                throw new \Exception('无法创建缓存路径');
            }
        }
        return $file_cache_path . md5($key);
    }
}